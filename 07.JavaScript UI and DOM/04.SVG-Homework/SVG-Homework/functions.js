...
function addImageExtended(posX, posY, src) {
    var image = '<image xlink:href="';
    image += src;
    image += '" x="'
    image += posX;
    image += '" y="';
    image += posY;
    image += '" height="100px" width="205px"/>';
...
function createSmall(posX, posY, color, text, icon) {
    ...
    if (icon != undefined) {
        result += addImage(posX + 20, posY + 20, icon);
    }
...
function createBig(posX, posY, color, text, icon) {
    ...
    if (icon != undefined) {
        result += addImage(posX + 80, posY + 20, icon);
    }

    return result;
}

function createBigExtended(posX, posY, color, text, icon) {
    var result = createRect(posX, posY, 205, 100, color);
    result += createText(posX + 10, posY + 90, text);

    if (icon != undefined) {
        result += addImageExtended(posX, posY, icon);
    }
